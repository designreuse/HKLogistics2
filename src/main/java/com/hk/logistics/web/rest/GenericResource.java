...
public class GenericResource {
	...
	public PincodeDeliveryInfoResponse estimatedDeliverDate(
			@Valid @RequestBody PincodeDeliveryInfoRequest pincodeDeliveryInfoRequest) {
		return variantService.getVariantDeliveryInfoByPincode(pincodeDeliveryInfoRequest);
	}

	@PostMapping("/pincode/courier/fetchCouriers/{stId}/{channel}")
	@Timed
	public CourierResponse getAllCouriersName(@PathVariable(DtoJsonConstants.STORE_ID) String storeId,
		...
		List<CourierChannel> courierChannels = pincodeCourierService.getAllCourierListForStoreAndChannel(storeId,
				channel);
	...
	@Timed
	public String markAwbUnused(@PathVariable(DtoJsonConstants.COURIER_SHORT_CODE) String courierShortCode,
			@PathVariable(DtoJsonConstants.AWB_NO) String awbNumber, @PathVariable(DtoJsonConstants.STORE) String store,
			@PathVariable(DtoJsonConstants.CHANNEL) String channel,
			@PathVariable(DtoJsonConstants.VENDOR_CODE) String vendorCode) {
	...
	@Timed
	public String getShipmentPricingResponseForBrightShipment(
			@Valid @RequestBody ShipmentPricingRequest shipmentPricingRequest) {
		ShipmentPricingResponse shipmentPricingResponse = shipmentPricingEngine
				.getShipmentPricingResponseForBright(shipmentPricingRequest);
	...
	public AwbCourierResponse attachAwbForBrightCourierChange(
		...
		// awbCourierResponse.setOperationsBitset(courier.getOperationsBitset());TODO
		// to be removed
	...
	public String validateAwb(@PathVariable(DtoJsonConstants.COURIER_SHORT_CODE) String courierShortCode,
			...
			@PathVariable(DtoJsonConstants.IS_COD) String isCod, @PathVariable(DtoJsonConstants.STORE) String store,
			@PathVariable(DtoJsonConstants.CHANNEL) String channel,
			@PathVariable(DtoJsonConstants.FULFILLMENT_CENTRE_CODE) String fulfillmentCentreCode) {
		...
		try {
			if (awbNumber == null || courierShortCode == null) {
				...
				healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
						"Awb no is already used!!Please use another awb!!");
			} else {
				...
				if (courier != null) {
					...
					if (awb != null) {
						if (awb.getAwbStatus().equals(EnumAwbStatus.Used.getAsAwbStatus())) {
							...
							healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
									"Awb no is already attached with some order!!Please use another awb!!");
						} else {
						...
						healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
								"Awb no does not exist!!Please use another awb!!");
					}
				} else {
			...
			log.error("Exception occurred while validating " + awbNumber + " awb number for " + courierShortCode
					+ " courier !!", ex);
			ex.printStackTrace();
			healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
					"Exception occurred while validating " + awbNumber + " awb number for " + courierShortCode
							+ " courier !!");
		}
	...
	public String markAwbUsed(@PathVariable(DtoJsonConstants.COURIER_SHORT_CODE) String courierShortCode,
		...
		if (awbNumber == null || courierShortCode == null) {
			healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
					"Awb no or courier short Code cannot be null!!");
		} else {
			Courier courier = courierRepository.findByShortCode(courierShortCode);
			if (courier != null) {
				...
				if (awb != null) {
					if (awb.getAwbStatus().equals(EnumAwbStatus.Attach.getAsAwbStatus())) {
						...
						healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
								"Awb no is already used with some order!!Please use another awb!!");
					} else {
						awb.setAwbStatus(EnumAwbStatus.Used.getAsAwbStatus());
						healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_OK,
								"Awb mark as used successfully.");
					}
				} else {
					healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR,
							"Awb no does not exist!!");
				}
			} else {
				healthkartResponse = new HealthkartResponse(HealthkartResponse.STATUS_ERROR, "Courier not found!!");
			}
	...
	public StoreVariantAPIObj getVariantServiceabilityDetails(
			@Valid @RequestBody VariantServiceabilityRequest variantServiceabilityRequest) {
		StoreVariantAPIObj storeVariantAPIObj = variantService
				.getVariantServiceabilityDetails(variantServiceabilityRequest);
	...
	public String getServiceabilityForReturnRequest(ReturnServiceabilityRequest returnServiceabilityRequest) {
		Boolean isReturnAvailable = pincodeCourierService
				.checkIfReturnServiceabilityAvailable(returnServiceabilityRequest);
	...
	@Timed
	public String preserveBrightAwb(@PathVariable(DtoJsonConstants.AWB_NO) String awbNumber,
			@PathVariable(DtoJsonConstants.STORE) String store) {
		...
		Gson gson = new Gson();
		return ((awb != null && awb.getAwbStatus().getId().equals(EnumAwbStatus.Unused.getId()))
				? gson.toJson(Boolean.TRUE) : gson.toJson(Boolean.FALSE));
	}
}
